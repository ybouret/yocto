...
YOCTO_UNIT_TEST_IMPL(ipso_opt)
{
    ...
    if(true)
    {
        ...
        coord1D fallback = 0;
        coord1D sizes = partition<coord1D>::optimal(cpus,ng,zone,pbcs.x,&fallback,stderr);
    ...
    if(true)
    {
        ...
        coord2D fallback;
        coord2D sizes = partition<coord2D>::optimal(cpus,ng,zone,coord2D(pbcs.x,pbcs.y),&fallback,stderr);
        std::cerr << "opt_sizes=" << sizes << ", fallback=" << fallback << std::endl;
    }
    ...
    if(true)
    {
        ...
        coord3D fallback;
        coord3D sizes = partition<coord3D>::optimal(cpus,ng,zone,pbcs,&fallback,stderr);
        std::cerr << "opt_sizes=" << sizes << ", fallback=" << fallback << std::endl;
    }

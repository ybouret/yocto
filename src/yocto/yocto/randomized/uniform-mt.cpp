...
#define               MATRIX_A      0x9908b0dfUL  /* constant vector a */
        ...
        void UniformMT:: initialize(const unsigned long u) throw()
        {
            mt[0]= u & 0xffffffffUL;
            for(mti=1; mti<N; mti++)
            {
        ...
        uint32_t UniformMT:: next32() throw()
        {
            return genrand_uint32(mti,mt);
        }

        void UniformMT:: reseed(Bits &bits) throw()
        {
            initialize( bits.full<unsigned long>() );
        }

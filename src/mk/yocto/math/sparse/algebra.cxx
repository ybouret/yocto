...
namespace yocto
{
    namespace math
    {
        ...
        template <>
        void sp_algebra<z_type>::mul_trn(array<z_type> &v, const sp_matrix<z_type> &M, const array<z_type> &u) throw()
        {
            static const z_type __zero__ = numeric<z_type>::zero;
            ...
            {
                const item_type &it = M[i];
                v[it.col] += it.value * u[it.row];
            }
        }
        
        template <>
        void sp_algebra<z_type>::muladd_trn(array<z_type> &v, const sp_matrix<z_type> &M, const array<z_type> &u) throw()
        {
            assert(v.size()==M.cols);
            assert(u.size()==M.rows);
        ...
        template <>
        void sp_algebra<z_type>::mulsub_trn(array<z_type> &v, const sp_matrix<z_type> &M, const array<z_type> &u) throw()
        {
            assert(v.size()==M.cols);
            assert(u.size()==M.rows);

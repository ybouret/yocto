...
static inline void __test_sp()
{
    ...
    {
        sp_matrix<T> S(1+alea.leq(n),1+alea.leq(n));
        const size_t n2 = S.rows * S.cols;
        for(size_t k=n2/2;k>0;--k)
        {
            const size_t i = 1 + alea.lt(S.rows);
            const size_t j = 1 + alea.lt(S.cols);
            S(i,j) = T(alea.to<float>());
        }


#include "yocto/ink/ops/histogram.hpp"
#include "yocto/math/stat/int-hist.hpp"
#include "yocto/sequence/lw-array.hpp"

namespace yocto
{
    namespace Ink
    {

        Histogram:: ~Histogram() throw()
        {
        }

        Histogram:: Histogram() throw() :
        counts(),
        source(0),
        target(0),
        proc(0),
        _level(0),
        _kmode(KeepGEQ),
        _invpx(false)
        {
            clear();
        }

        void Histogram:: clear() throw()
        {
            memset(counts,0,sizeof(counts));
        }

        void Histogram:: collect( const count_t *other ) throw()
        {
            assert(other);
            for(size_t i=0;i<BINS;++i)
            {
                counts[i] += other[i];
            }
        }

        void Histogram:: collectFrom( const Engine &engine) throw()
        {
            for(const Domain *dom = engine.head(); dom; dom=dom->next )
            {
                assert(dom->cache.data);
                assert(dom->cache.size>=BytesPerDomain);
                collect( static_cast<const count_t *>(dom->cache.data) );
            }
        }

        size_t Histogram:: threshold() const throw()
        {
            const lw_array<uint8_t> __bins( (uint8_t *)&Bins[0],BINS);
            const lw_array<count_t> __hist( (count_t *)&counts[0],BINS);
            return math::OtsuThreshold(__bins,__hist);
        }

        const uint8_t Histogram:: Bins[Histogram::BINS] =
        {
            
        };
    }
}

